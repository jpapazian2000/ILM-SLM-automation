resource "boundary_host_static" "first" {
  ...
  address         = data.terraform_remote_state.aws_ec2.outputs.ec2_first_addr
...
resource "boundary_host_static" "second" {
  ...
  address         = data.terraform_remote_state.aws_ec2.outputs.ec2_second_addr
  host_catalog_id = boundary_host_catalog_static.ec2-catalog.id
}
...
resource "boundary_host_static" "rds_host" {
  ...
  address = data.terraform_remote_state.aws_infra.outputs.db_host

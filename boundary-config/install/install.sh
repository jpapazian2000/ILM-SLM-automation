...
[Service]
ExecStart=${NAME} server -config /etc/${NAME}-${TYPE}.hcl
User=boundary
Group=boundary
...
if [ "${TYPE}" = "controller" ]; then
  sudo boundary database init -skip-auth-method-creation -skip-host-resources-creation -skip-scopes-creation -skip-target-creation -config /etc/${NAME}-${TYPE}.hcl || true
fi
